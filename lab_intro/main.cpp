...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
        for (size_t xi = 0; xi < original.width(); xi++) {
            ...
            int newRed = (int)original(xi, yi)->red + amount;
            int newGreen = (int)original(xi, yi)->green + amount;
            int newBlue = (int)original(xi, yi)->blue + amount;

 	    if(newRed > 255) {original(xi, yi)->red = 255;}
	    else original(xi, yi)->red += amount;
	    if(newGreen > 255) {original(xi, yi)->green = 255;}
            else original(xi, yi)->green += amount;
            if(newBlue > 255) {original(xi,yi)->blue = 255;}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
    ...
    for (size_t yi = 0; yi < firstImage.height(); yi++){
	    ...
	    int avgRed = (firstImage(xi, yi)->red + secondImage(xi, yi)->red ) / 2;
	    int avgGreen = (firstImage(xi, yi)->green + secondImage(xi, yi)->green) / 2;
	    int avgBlue = (firstImage(xi, yi)->blue + secondImage(xi, yi)->blue) / 2;
